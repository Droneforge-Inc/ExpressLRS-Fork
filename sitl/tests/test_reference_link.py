"""Actual native codecs/queues and negotiated v2 lifecycle; no hardware."""
import argparse
import struct
import sys
...
def reference(epoch,seq,t,inactive):
    body=struct.pack('!BBHHI9hhH',1,int(inactive),epoch,seq,t//1000,*([0]*10),200)
    f=bytes([0xee,36,0xd5,0xc8,0xea])+body
    return f+bytes([crc8(f[2:])])

def scenario(bin_dir,kind):
    received=[];status=[];markers=0;uart=bytearray();epoch=7;seq=0
    with Peer(bin_dir/'elrs_sitl_tx') as tx,Peer(bin_dir/'elrs_sitl_rx') as rx:
        for peer in (tx,rx):
            ...
            peer.call(ENABLE_UPLINK,payload=b'' if kind=='legacy_rx' and peer is rx else b'\x02')
        for t in range(0,8000000,2000):
            ...
            armed=kind=='rx_reset' and 1000000<=t<4000000
...
def rejected_requests_preserve_stream(bin_dir):
    ...
    with Peer(bin_dir / 'elrs_sitl_tx') as tx:
        ...
        for body in (b'\x00', b'\x01', b'\x03', b'\x02\x00'):
            try:
        ...
        tx.call(ENABLE_UPLINK, payload=b'\x02')
        for body in (b'', b'\x02'):
            try:
                tx.call(ENABLE_UPLINK, payload=body)
            except ValueError:
...
if __name__=='__main__':
    ...
    for kind in ('legacy_rx','rx_reset','epoch_reset'):scenario(args.bin_dir,kind)
    for mode,ratio in ((1,2),(0,4)):
        with Peer(args.bin_dir/'elrs_sitl_tx') as tx:
            ...
            try:tx.call(ENABLE_UPLINK,payload=b'\x02')
